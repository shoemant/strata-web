...
export default function LoginFlowPage() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      if (!profile?.full_name) {
        router.replace(`/onboarding/profile?returnTo=${encodeURIComponent(target)}`);
      } else {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const userId = await signInWithPassword(email, password, rememberMe);
      if (!userId) return;

      const { profile, buildingId } = await getProfileWithBuilding(userId);
      const target = landingPath(profile?.role, buildingId);

      if (!profile?.full_name) return router.push(`/onboarding/profile?returnTo=${encodeURIComponent(target)}`);
