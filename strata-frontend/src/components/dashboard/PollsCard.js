...
export default function PollsCard({
  buildingId,
  basePath, // "owner" or "manager"
  openPollsCount = 0,
  className = '',
}) {
  const href = buildingId
    ? `/${basePath}/buildings/${buildingId}/polls`
    : `/${basePath}/polls`;

  return (
    <Card
      className={`rounded-xl border-border/60 shadow-sm hover:shadow-md transition-shadow ${className}`}
    >
          ...
          <div className="flex items-center justify-between rounded-xl border border-border/50 p-4">
            <div className="min-w-0">
              ...
              <div className="text-sm text-muted-foreground">
                You have {openPollsCount} poll{openPollsCount === 1 ? '' : 's'}{' '}
                to vote on.
              </div>
            </div>
