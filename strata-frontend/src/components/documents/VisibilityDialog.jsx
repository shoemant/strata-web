...
export function VisibilityDialog({ open, folder, onClose, supabase, buildingId }) {
...
if (visErr) {
  ...
  setSelectedUnits(safeVis.filter(v => v.unit_id).map(v => v.unit_id));
}
...
setUnits(unitsData || []);   // safe default
  ...
  const handleSave = async () => {
    ...
    // wipe old
    await supabase.from("document_visibility").delete().eq("document_id", folder.id);
